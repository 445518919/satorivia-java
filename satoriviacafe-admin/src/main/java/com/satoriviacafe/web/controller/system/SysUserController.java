...
import com.satoriviacafe.system.service.ISysPostService;
...
public class SysUserController extends BaseController {
    ...
    public AjaxResult getInfo(@PathVariable(value = "userId", required = false) Long userId) {
        ...
        if (VStringUtils.isNotNull(userId)) {
    ...
    public AjaxResult add(@Validated @RequestBody SysUser user) {
        ...
        if (!userService.checkUserNameUnique(user)) {
            return error("新增用户'%s'失败，登录账号已存在".formatted(user.getUserName()));
        } else if (VStringUtils.isNotEmpty(user.getPhonenumber()) && !userService.checkPhoneUnique(user)) {
            return error("新增用户'%s'失败，手机号码已存在".formatted(user.getUserName()));
        } else if (VStringUtils.isNotEmpty(user.getEmail()) && !userService.checkEmailUnique(user)) {
            return error("新增用户'%s'失败，邮箱账号已存在".formatted(user.getUserName()));
        }
    ...
    public AjaxResult edit(@Validated @RequestBody SysUser user) {
        ...
        roleService.checkRoleDataScope(user.getRoleIds());
        if (!userService.checkUserNameUnique(user)) {
            return error("修改用户'%s'失败，登录账号已存在".formatted(user.getUserName()));
        } else if (VStringUtils.isNotEmpty(user.getPhonenumber()) && !userService.checkPhoneUnique(user)) {
            return error("修改用户'%s'失败，手机号码已存在".formatted(user.getUserName()));
        } else if (VStringUtils.isNotEmpty(user.getEmail()) && !userService.checkEmailUnique(user)) {
            return error("修改用户'%s'失败，邮箱账号已存在".formatted(user.getUserName()));
        }
