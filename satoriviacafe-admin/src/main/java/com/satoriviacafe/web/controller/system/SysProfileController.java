...
import com.satoriviacafe.common.utils.file.MimeTypeUtils;
...
public class SysProfileController extends BaseController {
    ...
    public AjaxResult updateProfile(@RequestBody SysUser user) {
        ...
        if (VStringUtils.isNotEmpty(user.getPhonenumber()) && !userService.checkPhoneUnique(currentUser)) {
            return error("修改用户'%s'失败，手机号码已存在".formatted(loginUser.getUsername()));
        }
        if (VStringUtils.isNotEmpty(user.getEmail()) && !userService.checkEmailUnique(currentUser)) {
            return error("修改用户'%s'失败，邮箱账号已存在".formatted(loginUser.getUsername()));
        }
    ...
    public AjaxResult avatar(@RequestParam("avatarfile") MultipartFile file) throws Exception {
        if (!file.isEmpty()) {
            LoginUser loginUser = getLoginUser();
            String avatar = FileUploadUtils.upload(SatoriviacafeConfig.getAvatarPath(), file, MimeTypeUtils.IMAGE_EXTENSION);
