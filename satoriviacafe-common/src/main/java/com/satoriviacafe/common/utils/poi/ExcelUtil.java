package com.satoriviacafe.common.utils.poi;

import com.satoriviacafe.common.annotation.Excel;
import com.satoriviacafe.common.annotation.Excel.ColumnType;
import com.satoriviacafe.common.annotation.Excel.Type;
import com.satoriviacafe.common.annotation.Excels;
...
import com.satoriviacafe.common.exception.UtilException;
import com.satoriviacafe.common.utils.DateUtils;
import com.satoriviacafe.common.utils.DictUtils;
import com.satoriviacafe.common.utils.VStringUtils;
import com.satoriviacafe.common.utils.file.FileTypeUtils;
import com.satoriviacafe.common.utils.file.FileUtils;
import com.satoriviacafe.common.utils.file.ImageUtils;
import com.satoriviacafe.common.utils.reflect.ReflectUtils;
 ...
 */
@SuppressWarnings({"FieldCanBeLocal", "LoggingSimilarMessage", "DuplicatedCode", "UnusedReturnValue", "unused"})
public class ExcelUtil<T> {
    ...
    public void createTitle() {
        if (VStringUtils.isNotEmpty(title)) {
        ...
        if (isSubList()) {
            ...
            for (Object[] objects : fields) {
                ...
                if (Collection.class.isAssignableFrom(field.getType())) {
                    ...
                    cell.setCellStyle(styles.get(VStringUtils.format("header_{}_{}", attr.headerColor(), attr.headerBackgroundColor())));
                    if (subFieldSize > 1) {
    ...
    public List<T> importExcel(InputStream is, int titleNum) {
        List<T> list;
        try {
            list = importExcel(VStringUtils.EMPTY, is, titleNum);
        } catch (Exception e) {
    ...
    public List<T> importExcel(String sheetName, InputStream is, int titleNum) throws Exception {
        ...
        Sheet sheet = VStringUtils.isNotEmpty(sheetName) ? wb.getSheet(sheetName) : wb.getSheetAt(0);
        if (sheet == null) {
        ...
        if (rows > 0) {
            ...
            for (int i = 0; i < heard.getPhysicalNumberOfCells(); i++) {
                ...
                if (VStringUtils.isNotNull(cell)) {
            ...
            for (int i = titleNum + 1; i <= rows; i++) {
                ...
                for (Map.Entry<Integer, Object[]> entry : fieldsMap.entrySet()) {
                    ...
                    if (String.class == fieldType) {
                        ...
                        if (VStringUtils.endsWith(s, ".0")) {
                            val = VStringUtils.substringBefore(s, ".0");
                        } else {
                            String dateFormat = field.getAnnotation(Excel.class).dateFormat();
                            if (VStringUtils.isNotEmpty(dateFormat)) {
                                val = parseDateToStr(dateFormat, val);
                            } else {
                                val = Convert.toStr(val);
                            }
                        }
                    } else if ((Integer.TYPE == fieldType || Integer.class == fieldType) && VStringUtils.isNumeric(Convert.toStr(val))) {
                        val = Convert.toInt(val);
                    } else if ((Long.TYPE == fieldType || Long.class == fieldType) && VStringUtils.isNumeric(Convert.toStr(val))) {
                    ...
                    if (VStringUtils.isNotEmpty(attr.targetAttr())) {
                        propertyName = field.getName() + "." + attr.targetAttr();
                    }
                    if (VStringUtils.isNotEmpty(attr.readConverterExp())) {
                        val = reverseByExp(Convert.toStr(val), attr.readConverterExp(), attr.separator());
                    } else if (VStringUtils.isNotEmpty(attr.dictType())) {
                    ...
                    } else if (ColumnType.IMAGE == attr.cellType() && VStringUtils.isNotEmpty(pictures)) {
    ...
    public AjaxResult exportExcel(List<T> list, String sheetName) {
        return exportExcel(list, sheetName, VStringUtils.EMPTY);
    }
    ...
    public void exportExcel(HttpServletResponse response, List<T> list, String sheetName) {
        exportExcel(response, list, sheetName, VStringUtils.EMPTY);
    }
    ...
    public AjaxResult importTemplateExcel(String sheetName) {
        return importTemplateExcel(sheetName, VStringUtils.EMPTY);
    }
    ...
    public void importTemplateExcel(HttpServletResponse response, String sheetName) {
        importTemplateExcel(response, sheetName, VStringUtils.EMPTY);
    }
    ...
    private Map<String, CellStyle> annotationHeaderStyles(Workbook wb, Map<String, CellStyle> styles) {
        ...
        for (Object[] os : fields) {
            Excel excel = (Excel) os[1];
            String key = VStringUtils.format("header_{}_{}", excel.headerColor(), excel.headerBackgroundColor());
    ...
    public void annotationDataStyles(Map<String, CellStyle> styles, Field field, Excel excel) {
        String key = VStringUtils.format("data_{}_{}_{}_{}_{}", excel.align(), excel.color(), excel.backgroundColor(), excel.cellType(), excel.wrapText());
    ...
    public Cell createHeadCell(Excel attr, Row row, int column) {
        ...
        if (isSubList()) {
            // 填充默认样式，防止合并单元格样式失效
            sheet.setDefaultColumnStyle(column, styles.get(VStringUtils.format("data_{}_{}_{}_{}_{}", attr.align(), attr.color(), attr.backgroundColor(), attr.cellType(), attr.wrapText())));
            if (attr.needMerge()) {
    ...
    public void setCellVo(Object value, Excel attr, Cell cell) {
        if (ColumnType.STRING == attr.cellType() || ColumnType.TEXT == attr.cellType()) {
            ...
            if (VStringUtils.startsWithAny(cellValue, FORMULA_STR)) {
                cellValue = RegExUtils.replaceFirst(cellValue, FORMULA_REGEX_STR, "\t$0");
            }
            if (value instanceof Collection && VStringUtils.equals("[]", cellValue)) {
                cellValue = VStringUtils.EMPTY;
            }
            cell.setCellValue(VStringUtils.isNull(cellValue) ? attr.defaultValue() : cellValue + attr.suffix());
        } else if (ColumnType.NUMERIC == attr.cellType()) {
            if (VStringUtils.isNotNull(value)) {
                cell.setCellValue(VStringUtils.contains(Convert.toStr(value), ".") ? Convert.toDouble(value) : Convert.toInt(value));
            }
        } else if (ColumnType.IMAGE == attr.cellType()) {
            ClientAnchor anchor = new XSSFClientAnchor(0, 0, 0, 0, (short) cell.getColumnIndex(), cell.getRow().getRowNum(), (short) (cell.getColumnIndex() + 1), cell.getRow().getRowNum() + 1);
            String imagePath = Convert.toStr(value);
            if (VStringUtils.isNotEmpty(imagePath)) {
    ...
    public void setDataValidation(Excel attr, Row row, int column) {
        ...
        if (VStringUtils.isNotEmpty(attr.prompt()) || attr.combo().length > 0 || attr.comboReadDict()) {
            ...
            if (attr.comboReadDict()) {
                ...
                comboArray = VStringUtils.split(val, DictUtils.SEPARATOR);
            }
            if (comboArray.length > 15 || VStringUtils.join(comboArray).length() > 255) {
    ...
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column) {
        ...
        try {
            ...
            if (attr.isExport()) {
                ...
                cell.setCellStyle(styles.get(VStringUtils.format("data_{}_{}_{}_{}_{}", attr.align(), attr.color(), attr.backgroundColor(), attr.cellType(), attr.wrapText())));
                ...
                if (VStringUtils.isNotEmpty(dateFormat) && VStringUtils.isNotNull(value)) {
                    cell.getCellStyle().setDataFormat(this.wb.getCreationHelper().createDataFormat().getFormat(dateFormat));
                    cell.setCellValue(parseDateToStr(dateFormat, value));
                } else if (VStringUtils.isNotEmpty(readConverterExp) && VStringUtils.isNotNull(value)) {
                    cell.setCellValue(convertByExp(Convert.toStr(value), readConverterExp, separator));
                } else if (VStringUtils.isNotEmpty(dictType) && VStringUtils.isNotNull(value)) {
    ...
    public void setPromptOrValidation(Sheet sheet, String[] textlist, String promptContent, int firstRow, int endRow,
        ...
        if (VStringUtils.isNotEmpty(promptContent)) {
            // 如果设置了提示信息则鼠标放上去提示
    ...
    public static String convertByExp(String propertyValue, String converterExp, String separator) {
        ...
        for (String item : convertSource) {
            ...
            if (VStringUtils.containsAny(propertyValue, separator)) {
    ...
    public String getAbsoluteFile(String filename) {
        String downloadPath = SatoriviacafeConfig.getDownloadPath() + filename;
    ...
    private Object getTargetValue(T vo, Field field, Excel excel) throws Exception {
        ...
        if (VStringUtils.isNotEmpty(excel.targetAttr())) {
    ...
    private Object getValue(Object o, String name) throws Exception {
        if (VStringUtils.isNotNull(o) && VStringUtils.isNotEmpty(name)) {
    ...
    public boolean isSubList() {
        return VStringUtils.isNotNull(subFields) && !subFields.isEmpty();
    }
    ...
    public boolean isSubListValue(T vo) {
        return VStringUtils.isNotNull(subFields) && !subFields.isEmpty() && VStringUtils.isNotNull(getListCellValue(vo)) && !getListCellValue(vo).isEmpty();
    }
